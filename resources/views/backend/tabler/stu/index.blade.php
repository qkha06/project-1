              ...
              <tbody>
                ...
                @foreach($user_stu_links as $key=>$value)
                    <tr>
                        ...
                        @endif
                        <td style="white-space: nowrap"><a href="{{ ($_settings['stu_url'] ?? env('APP_URL')).'/'.$value->alias }}" target="__blank">{{ ($_settings['stu_url'] ?? env('APP_URL')).'/'.$value->alias }}</a></td>
                        ...
                        @php
                        if ($value->level_id && $value->level) {
                            switch ($value->level_id) {
                                case 0:
                                    $elt_lv = '<span class="badge bg-azure-lt">'.$value->level->name.'</span>';
                                    break;
                                case 1:
                                    $elt_lv = '<span class="badge bg-azure-lt">'.$value->level->name.'</span>';
                                    break;
